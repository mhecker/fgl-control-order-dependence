...
showSimpleTwoValueTransitionSystem = do
    ...
    let p = (toProgramSimple $ SimpleProgram (Map.fromList [(1, "1")])  (Map.fromList [("1",Generated (Seq Skip (If (Leq (Val 0) (Times (Var (Global "y")) (Var (Global "x")))) (Ass (Global "z") (Times (Var (Global "z")) (Var (Global "x")))) (Ass (Global "h") (Times (Var (Global "h")) (Var (Global "x")))))) (Set.map (VarName . Global) $ Set.fromList ["h","x","y","z"]) (Map.fromList []) (Map.empty))])) :: Program Gr
            ...
            (ForV (Global "y")
               (ForV (Global "x")
                  (Seq (ReadFromChannel (Global "z") "stdIn")
                        Skip))))
       (Set.map (VarName . Global) $ Set.fromList ["a","x","y"])
       (Map.fromList [(2,  Set.map (VarName . Global) $ Set.fromList ["a","y"])])
       (Map.empty)
    ),
    ("2",Generated
            (ForV (Global "a")
       ...
       (Set.map (VarName . Global) $ Set.fromList ["a","y"])
       (Map.fromList [])
       (Map.empty)
    )
    ])
...
someGen12 = IntraGeneratedProgram
    (Map.fromList [(1,"main"),(2,"thread2"),(3,"thread3")])
    (Map.fromList [("main",Generated (Seq (Seq (If CTrue (Seq (PrintToChannel (Val 4) "stdOut") (SpawnThread 2)) (Seq (PrintToChannel (Val 1) "stdOut") Skip)) (Seq (ForC 1 (PrintToChannel (Val 0) "stdOut")) (Seq (PrintToChannel (Val 9) "stdOut") (SpawnThread 3)))) (If CTrue (Seq (Seq Skip (PrintToChannel (Val 0) "stdOut")) (Seq (PrintToChannel (Val (-1)) "stdOut") (PrintToChannel (Val 9) "stdOut"))) (If CFalse (ForC 1 (ReadFromChannel (Global "z") "stdIn")) (ForC 1 Skip)))) undefined undefined undefined),("thread2",Generated (ForC 1 (Seq (If CFalse (Seq (If CFalse (PrintToChannel (Val 1) "stdOut") (Ass (Global "a") (Val 4))) (Seq (PrintToChannel (Val 9) "stdOut") (PrintToChannel (Val (-1)) "stdOut"))) (Seq (Seq (Ass (Global "y") (Val 1)) Skip) (Seq (Ass (Global "a") (Plus (Var (Global "y")) (Var (Global "y")))) (Ass (Global "x") (Var (Global "y")))))) (Seq (Seq (ForC 2 (Ass (Global "c") (Val 1))) (Seq (PrintToChannel (Val 9) "stdOut") (ReadFromChannel (Global "z") "stdIn"))) (Seq (ForV (Global "c") (Ass (Global "x") (Neg (Var (Global "z"))))) (If (Leq (Val 0) (Plus (Var (Global "c")) (Var (Global "c")))) (ReadFromChannel (Global "a") "stdIn") (ReadFromChannel (Global "c") "stdIn")))))) undefined undefined undefined),("thread3",Generated (Seq (If CTrue (Seq (PrintToChannel (Val 9) "stdOut") (Ass (Global "z") (Val 1))) (ForC 1 (PrintToChannel (Val (-1)) "stdOut"))) (If CTrue (Seq Skip Skip) (ForC 2 (PrintToChannel (Val 0) "stdOut")))) undefined undefined undefined)])
